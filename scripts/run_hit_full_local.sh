#!/usr/bin/env bash
# Local full-scale HIT run (eager IsoType). One model at a time; resumable.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
BIN="$ROOT/target/release/opendefalgsplitting"
CORPUS="$ROOT/benches/scale/corpus/full"
OUT="$ROOT/benches/scale/results/results_hit_full.csv"
LOG="$ROOT/benches/scale/results/hit_full_run.log"
export HIT_MAX_RSS_MIB="${HIT_MAX_RSS_MIB:-45000}"
...
python3 - <<'PY' > /tmp/hit_full_done.txt
import csv
from pathlib import Path
p = Path("benches/scale/results/results_hit_full.csv")
if not p.is_file():
    raise SystemExit
...
skip=0
i=0
echo "START total=$total already=${#SEEN[@]} HIT_MAX_RSS_MIB=$HIT_MAX_RSS_MIB" | tee -a "$LOG"
for m in "${models[@]}"; do
  i=$((i + 1))
  name="$(basename "$m")"
  if [[ -n "${SEEN[$name]:-}" ]]; then
  ...
  line=$(timeout --signal=TERM "$to_s" "$BIN" \
    --bench --repeat 1 --fragment qf --engine hit "$m" 2>>"$LOG" | awk -F'\t' 'NF>=3 && $1!="model" {print; exit}')
  ec=$?
  set -e
  if [[ $ec -eq 124 ]] || [[ $ec -eq 137 ]]; then
    echo "hit,$m,,TIMEOUT" >> "$OUT"
    to=$((to + 1))
    echo "[$i/$total] TIMEOUT $name" | tee -a "$LOG"
    continue
  fi
  if [[ -z "${line:-}" ]]; then
    echo "hit,$m,,ERROR" >> "$OUT"
    err=$((err + 1))
